...
use crate::{
    math::AproxEq, services::ServiceCycle
};
///
///
...
fn basic() {
    ...
    for target_cycle_interval in TARGET_CYCLE_INTERVALS {  // ms
        ...
        assert!((errors as f64) < errors_allowed, "to much errors ({}), a few errors will be ok, but not more then {}% of test cycles", errors, errors_allowed);
    }
}
