...
mod services {
    use std::{env, sync::{atomic::{AtomicBool, Ordering}, Arc, Once}, time::Duration};
    use coco::Stack;
    use sal_core::dbg::Dbg;
    use testing::stuff::{max_test_duration::TestDuration};
    use debugging::session::debug_session::{DebugSession, LogLevel, Backtrace};
    use crate::{services::{conf::{ConfTree, ServicesConf}, entity::{Name, Object}, Service, Services}, sync::WaitBox, thread_pool::Scheduler};
    ///
    ///
    ...
    fn services_thread() {
        ...
        // let target  = 3;
        // let points = services.points(dbg).then(|points| points, |err| {
        //     log::error!("{}.handle.Subscribe | Requesting points error: {:?}", dbg, err);
        //     vec![]
        // });
        // let points_count = points.len();
        // log::debug!(" points count: {:?}", points_count);
        // for point in points {
        //     log::debug!("\t {:?}", point);
        // }
    ...
    impl Service for ServiceMok {
        fn run(&self) -> Result<(), sal_core::error::Error> {
            ...
            let h = self.schrduler.as_ref().unwrap().spawn(move|| {
                loop {
